...
class GP:
    ...
    def prior_data(self):
        if self._prior_data is None:
            if self.prior_params is not None:
                prior_calc, _ = self.GetPriorCalc()
                self._prior_data = prior_calc(self._Xx.Values).reshape(-1,1)
                self._mu = None
            else:
                return 0 # No prior then prior is 0
        return self._prior_data
    def GetPriorCalc(self):
        if self.prior_params is not None:
            return SpherePriorCalculator(self.prior_params[0],self.prior_params[1],self.prior_params[2],self.prior_params[3],self.prior_params[4])
        return None 
    #endregion
    # region hyp
    _hyp = None
    ...
    def AddXx(self, x):
        ...
        if self._Xx.RescaleTables is not None:
            auxX.RescaleTables = self.InitializeRescaleTables(auxX)
        if self.prior_data is not None and self.prior_params is not None:
            prior_calc, _ = self.GetPriorCalc()
            self._prior_data = np.vstack((self._prior_data, prior_calc(x)))
    ...
    def GetCopy(self):
        ...
        newGP._K = self._K
        newGP._L = self._L
        newGP._prior_params = self._prior_params
        return newGP
